...
import { readTeamFileAsync } from '../../utils/swarm/teamHelpers.ts'
import {
  getAgentId,
  getAgentName,
  getTeammateColor,
      ...
      if (feature('UDS_INBOX') && typeof input.message === 'string') {
        ...
        if (addr.scheme === 'bridge') {
          ...
          const { postInterClaudeMessage } =
            require('../../bridge/peerSessions.ts') as typeof import('../../bridge/peerSessions.ts')
        ...
        if (addr.scheme === 'uds') {
          ...
          const { sendToUdsSocket } =
            require('../../utils/udsClient.ts') as typeof import('../../utils/udsClient.ts')
          /* eslint-enable @typescript-eslint/no-require-imports */
          try {
