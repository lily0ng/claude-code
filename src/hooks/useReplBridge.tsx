...
import { type BridgePermissionCallbacks, type BridgePermissionResponse, isBridgePermissionResponse } from '../bridge/bridgePermissionCallbacks.ts';
...
import type { BridgeState, ReplBridgeHandle } from '../bridge/replBridge.ts';
import { setReplBridgeHandle } from '../bridge/replBridgeHandle.ts';
import type { Command } from '../commands.ts';
import { getSlashCommandToolSkills, isBridgeSafeCommand } from '../commands.ts';
...
import { enqueue } from '../utils/messageQueueManager.ts';
import { buildSystemInitMessage } from '../utils/messages/systemInit.ts';
import { createBridgeStatusMessage, createSystemMessage } from '../utils/messages.ts';
import { getAutoModeUnavailableNotification, getAutoModeUnavailableReason, isAutoModeGateEnabled, isBypassPermissionsModeDisabled, transitionPermissionMode } from '../utils/permissions/permissionSetup.ts';
import { getLeaderToolUseConfirmQueue } from '../utils/swarm/leaderPermissionBridge.ts';
    ...
    if (feature('BRIDGE_MODE')) {
      ...
      void (async () => {
        try {
          ...
          const {
            initReplBridge
          } = await import('../bridge/initReplBridge.ts');
          const {
            shouldShowAppUpgradeMessage
          } = await import('../bridge/envLessBridgeConfig.ts');
          ...
          async function handleInboundMessage(msg: SDKMessage): Promise<void> {
            try {
              ...
              const {
                resolveAndPrepend
              } = await import('../bridge/inboundAttachments.ts');
              let sanitized = fields.content;
              if (feature('KAIROS_GITHUB_WEBHOOKS')) {
                /* eslint-disable @typescript-eslint/no-require-imports */
                const {
                  sanitizeInboundWebhookContent
                } = require('../bridge/webhookSanitizer.ts') as typeof import('../bridge/webhookSanitizer.ts');
