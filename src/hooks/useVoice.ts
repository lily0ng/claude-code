...
import { getVoiceKeyterms } from '../services/voiceKeyterms.ts'
import {
  connectVoiceStream,
  type FinalizeSource,
  isVoiceStreamAvailable,
  type VoiceStreamConnection,
} from '../services/voiceStreamSTT.ts'
import { logForDebugging } from '../utils/debug.ts'
import { toError } from '../utils/errors.ts'
import { getSystemLocaleLanguage } from '../utils/intl.ts'
...
type VoiceModule = typeof import('../services/voice.ts')
...
export function useVoice({
  ...
  useEffect(() => {
    if (enabled && !voiceModule) {
      void import('../services/voice.ts').then(mod => {
        voiceModule = mod
      })
    }
  }, [enabled])
