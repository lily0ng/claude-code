...
import { type Command, getCommandName } from '../commands.ts';
import { getModeFromInput, getValueFromInput } from '../components/PromptInput/inputModes.ts';
import type { SuggestionItem, SuggestionType } from '../components/PromptInput/PromptInputFooterSuggestions.ts';
import { useIsModalOverlayActive, useRegisterOverlay } from '../context/overlayContext.ts';
import { KeyboardEvent } from '../ink/events/keyboard-event.ts';
// eslint-disable-next-line custom-rules/prefer-use-keybindings -- backward-compat bridge until consumers wire handleKeyDown to <Box onKeyDown>
import { useInput } from '../ink.ts';
import { useOptionalKeybindingContext, useRegisterKeybindingContext } from '../keybindings/KeybindingContext.ts';
import { useKeybindings } from '../keybindings/useKeybinding.ts';
import { useShortcutDisplay } from '../keybindings/useShortcutDisplay.ts';
import { useAppState, useAppStateStore } from '../state/AppState.ts';
import type { AgentDefinition } from '../tools/AgentTool/loadAgentsDir.ts';
import type { InlineGhostText, PromptInputMode } from '../types/textInputTypes.ts';
import { isAgentSwarmsEnabled } from '../utils/agentSwarmsEnabled.ts';
import { generateProgressiveArgumentHint, parseArguments } from '../utils/argumentSubstitution.ts';
import { getShellCompletions, type ShellCompletionType } from '../utils/bash/shellCompletion.ts';
import { formatLogMetadata } from '../utils/format.ts';
import { getSessionIdFromLog, searchSessionsByCustomTitle } from '../utils/sessionStorage.ts';
import { applyCommandSuggestion, findMidInputSlashCommand, generateCommandSuggestions, getBestCommandMatch, isCommandInput } from '../utils/suggestions/commandSuggestions.ts';
import { getDirectoryCompletions, getPathCompletions, isPathLikeToken } from '../utils/suggestions/directoryCompletion.ts';
import { getShellHistoryCompletion } from '../utils/suggestions/shellHistoryCompletion.ts';
import { getSlackChannelSuggestions, hasSlackMcpServer } from '../utils/suggestions/slackChannelSuggestions.ts';
import { TEAM_LEAD_NAME } from '../utils/swarm/constants.ts';
import { applyFileSuggestion, findLongestCommonPrefix, onIndexBuildComplete, startBackgroundCacheRefresh } from './fileSuggestions.ts';
import { generateUnifiedSuggestions } from './unifiedSuggestions.ts';
