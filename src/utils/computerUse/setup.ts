...
import { buildMcpToolName } from '../../services/mcp/mcpStringUtils.ts'
import type { ScopedMcpServerConfig } from '../../services/mcp/types.ts'

import { isInBundledMode } from '../bundledMode.ts'
import { CLI_CU_CAPABILITIES, COMPUTER_USE_MCP_SERVER_NAME } from './common.ts'
...
export function setupComputerUseMCP(): {
  ...
  const args = isInBundledMode()
    ? ['--computer-use-mcp']
    : [
        join(fileURLToPath(import.meta.url), '..', 'cli.ts'),
        '--computer-use-mcp',
      ]
