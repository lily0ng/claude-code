...
import { getPlanSlugCache, getSessionId } from '../bootstrap/state.ts'
...
export async function persistFileSnapshotIfRemote(): Promise<void> {
  ...
  try {
    ...
    const { recordTranscript } = await import('./sessionStorage.ts')
