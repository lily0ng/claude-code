...
let cachedMCModule: typeof import('./cachedMicrocompact.ts') | null = null
let cachedMCState: import('./cachedMicrocompact.ts').CachedMCState | null = null
let pendingCacheEdits:
  | import('./cachedMicrocompact.ts').CacheEditsBlock
  | null = null

async function getCachedMCModule(): Promise<
  typeof import('./cachedMicrocompact.ts')
> {
  if (!cachedMCModule) {
    cachedMCModule = await import('./cachedMicrocompact.ts')
  }
  return cachedMCModule
}

function ensureCachedMCState(): import('./cachedMicrocompact.ts').CachedMCState {
 ...
 */
export function getPinnedCacheEdits(): import('./cachedMicrocompact.ts').PinnedCacheEdits[] {
  if (!cachedMCState) {
    return []
  }
...
export function pinCacheEdits(
  ...
  block: import('./cachedMicrocompact.ts').CacheEditsBlock,
): void {
  if (cachedMCState) {
