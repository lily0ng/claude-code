...
import { checkHasTrustDialogAccepted, getCustomApiKeyStatus, getGlobalConfig, saveGlobalConfig } from './utils/config.ts';
import { updateDeepLinkTerminalPreference } from './utils/deepLink/terminalPreference.ts';
import { isEnvTruthy, isRunningOnHomespace } from './utils/envUtils.ts';
import { type FpsMetrics, FpsTracker } from './utils/fpsTracker.ts';
import { updateGithubRepoPathMapping } from './utils/githubRepoPathMapping.ts';
...
import { getSettingsWithAllErrors } from './utils/settings/allErrors.ts';
import { hasAutoModeOptIn, hasSkipDangerousModePermissionPrompt } from './utils/settings/settings.ts';
...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], claudeInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  ...
  if (await isQualifiedForGrove()) {
    const {
      GroveDialog
    } = await import('src/components/grove/Grove.ts');
  ...
  if (feature('KAIROS') || feature('KAIROS_CHANNELS')) {
    ...
    if (devChannels && devChannels.length > 0) {
      ...
      }] = await Promise.all([import('./services/mcp/channelAllowlist.ts'), import('./utils/auth.ts')]);
