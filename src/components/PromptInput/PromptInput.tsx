...
import { type AppState, useAppState, useAppStateStore, useSetAppState } from './state/AppState.ts';
...
import { companionReservedColumns } from '../../buddy/CompanionSprite.ts';
import { findBuddyTriggerPositions, useBuddyNotification } from '../../buddy/useBuddyNotification.ts';
import { FastModePicker } from '../../commands/fast/fast.ts';
import { isUltrareviewEnabled } from '../../commands/review/ultrareviewEnabled.ts';
import { getNativeCSIuTerminalDisplayName } from '../../commands/terminalSetup/terminalSetup.ts';
import { type Command, hasCommand } from '../../commands.ts';
import { useIsModalOverlayActive } from '../../context/overlayContext.ts';
import { useSetPromptOverlayDialog } from '../../context/promptOverlayContext.ts';
import { formatImageRef, formatPastedTextRef, getPastedTextRefNumLines, parseReferences } from '../../history.ts';
import type { VerificationStatus } from '../../hooks/useApiKeyVerification.ts';
import { type HistoryMode, useArrowKeyHistory } from '../../hooks/useArrowKeyHistory.ts';
import { useDoublePress } from '../../hooks/useDoublePress.ts';
import { useHistorySearch } from '../../hooks/useHistorySearch.ts';
import type { IDESelection } from '../../hooks/useIdeSelection.ts';
import { useInputBuffer } from '../../hooks/useInputBuffer.ts';
...
import { useTypeahead } from '../../hooks/useTypeahead.ts';
import type { BorderTextOptions } from '../../ink/render-border.ts';
import { stringWidth } from '../../ink/stringWidth.ts';
import { Box, type ClickEvent, type Key, Text, useInput } from '../../ink.ts';
...
import { abortPromptSuggestion, logSuggestionSuppressed } from '../../services/PromptSuggestion/promptSuggestion.ts';
import { type ActiveSpeculationState, abortSpeculation } from '../../services/PromptSuggestion/speculation.ts';
import { getActiveAgentForInput, getViewedTeammateTask } from '../../state/selectors.ts';
import { enterTeammateView, exitTeammateView, stopOrDismissAgent } from '../../state/teammateViewHelpers.ts';
...
import { isBackgroundTask } from '../../tasks/types.ts';
...
import type { BaseTextInputProps, PromptInputMode, VimMode } from '../../types/textInputTypes.ts';
...
import { getGlobalConfig, type PastedContent, saveGlobalConfig } from '../../utils/config.ts';
import { logForDebugging } from '../../utils/debug.ts';
import { parseDirectMemberMessage, sendDirectMemberMessage } from '../../utils/directMemberMessage.ts';
import type { EffortLevel } from '../../utils/effort.ts';
import { env } from '../../utils/env.ts';
import { errorMessage } from '../../utils/errors.ts';
import { isBilledAsExtraUsage } from '../../utils/extraUsage.ts';
import { getFastModeUnavailableReason, isFastModeAvailable, isFastModeCooldown, isFastModeEnabled, isFastModeSupportedByModel } from '../../utils/fastMode.ts';
import { isFullscreenEnvEnabled } from '../../utils/fullscreen.ts';
import type { PromptInputHelpers } from '../../utils/handlePromptSubmit.ts';
import { getImageFromClipboard, PASTE_THRESHOLD } from '../../utils/imagePaste.ts';
import type { ImageDimensions } from '../../utils/imageResizer.ts';
import { cacheImagePath, storeImage } from '../../utils/imageStore.ts';
import { isMacosOptionChar, MACOS_OPTION_SPECIAL_CHARS } from '../../utils/keyboardShortcuts.ts';
import { logError } from '../../utils/log.ts';
import { isOpus1mMergeEnabled, modelDisplayString } from '../../utils/model/model.ts';
import { setAutoModeActive } from '../../utils/permissions/autoModeState.ts';
import { cyclePermissionMode, getNextPermissionMode } from '../../utils/permissions/getNextPermissionMode.ts';
...
import { hasAutoModeOptIn } from '../../utils/settings/settings.ts';
import { findBtwTriggerPositions } from '../../utils/sideQuestion.ts';
import { findSlashCommandPositions } from '../../utils/suggestions/commandSuggestions.ts';
import { findSlackChannelPositions, getKnownChannelsVersion, hasSlackMcpServer, subscribeKnownChannels } from '../../utils/suggestions/slackChannelSuggestions.ts';
import { isInProcessEnabled } from '../../utils/swarm/backends/registry.ts';
import { syncTeammateMode } from '../../utils/swarm/teamHelpers.ts';
import type { TeamSummary } from '../../utils/teamDiscovery.ts';
...
import { findTokenBudgetPositions } from '../../utils/tokenBudget.ts';
import { findUltraplanTriggerPositions, findUltrareviewTriggerPositions } from '../../utils/ultraplan/keyword.ts';
import { AutoModeOptInDialog } from '../AutoModeOptInDialog.ts';
import { BridgeDialog } from '../BridgeDialog.ts';
import { ConfigurableShortcutHint } from '../ConfigurableShortcutHint.ts';
import { getVisibleAgentTasks, useCoordinatorTaskCount } from '../CoordinatorAgentStatus.ts';
import { getEffortNotificationText } from '../EffortIndicator.ts';
import { getFastIconString } from '../FastIcon.ts';
import { GlobalSearchDialog } from '../GlobalSearchDialog.ts';
import { HistorySearchDialog } from '../HistorySearchDialog.ts';
import { ModelPicker } from '../ModelPicker.ts';
import { QuickOpenDialog } from '../QuickOpenDialog.ts';
import TextInput from '../TextInput.ts';
import { ThinkingToggle } from '../ThinkingToggle.ts';
import { BackgroundTasksDialog } from '../tasks/BackgroundTasksDialog.ts';
import { shouldHideTasksFooter } from '../tasks/taskStatusUtils.ts';
import { TeamsDialog } from '../teams/TeamsDialog.ts';
import VimTextInput from '../VimTextInput.ts';
import { getModeFromInput, getValueFromInput } from './inputModes.ts';
import { FOOTER_TEMPORARY_STATUS_TIMEOUT, Notifications } from './Notifications.ts';
import PromptInputFooter from './PromptInputFooter.ts';
import type { SuggestionItem } from './PromptInputFooterSuggestions.ts';
import { PromptInputModeIndicator } from './PromptInputModeIndicator.ts';
import { PromptInputQueuedCommands } from './PromptInputQueuedCommands.ts';
import { PromptInputStashNotice } from './PromptInputStashNotice.ts';
import { useMaybeTruncateInput } from './useMaybeTruncateInput.ts';
import { usePromptInputPlaceholder } from './usePromptInputPlaceholder.ts';
import { useShowFastIconHint } from './useShowFastIconHint.ts';
import { useSwarmBanner } from './useSwarmBanner.ts';
import { isNonSpacePrintable, isVimModeEnabled } from './utils.ts';
type Props = {
  debug: boolean;
