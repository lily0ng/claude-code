...
async function main(): Promise<void> {
  ...
  const {
    profileCheckpoint
  } = await import('../utils/startupProfiler.ts');
  ...
  if (feature('DUMP_SYSTEM_PROMPT') && args[0] === '--dump-system-prompt') {
    ...
    const {
      getMainLoopModel
    } = await import('../utils/model/model.ts');
    const modelIdx = args.indexOf('--model');
    const model = modelIdx !== -1 && args[modelIdx + 1] || getMainLoopModel();
    const {
      getSystemPrompt
    } = await import('../constants/prompts.ts');
  ...
  if (process.argv[2] === '--claude-in-chrome-mcp') {
    ...
    } = await import('../utils/claudeInChrome/mcpServer.ts');
    ...
    const {
      runChromeNativeHost
    } = await import('../utils/claudeInChrome/chromeNativeHost.ts');
    ...
    } = await import('../utils/computerUse/mcpServer.ts');
  ...
  if (feature('DAEMON') && args[0] === '--daemon-worker') {
    const {
      runDaemonWorker
    } = await import('../daemon/workerRegistry.ts');
  ...
  if (feature('BRIDGE_MODE') && (args[0] === 'remote-control' || args[0] === 'rc' || args[0] === 'remote' || args[0] === 'sync' || args[0] === 'bridge')) {
    ...
    const {
      BRIDGE_LOGIN_ERROR
    } = await import('../bridge/types.ts');
    const {
      bridgeMain
    } = await import('../bridge/bridgeMain.ts');
    const {
      exitWithError
    } = await import('../utils/process.ts');
    ...
    const {
      ...
      isPolicyAllowed
    } = await import('../services/policyLimits/index.ts');
  ...
  if (feature('TEMPLATES') && (args[0] === 'new' || args[0] === 'list' || args[0] === 'reply')) {
    ...
    const {
      templatesMain
    } = await import('../cli/handlers/templateJobs.ts');
  ...
  if (hasTmuxFlag && (args.includes('-w') || args.includes('--worktree') || args.some(a => a.startsWith('--worktree=')))) {
    ...
    } = await import('../utils/worktreeModeEnabled.ts');
    if (isWorktreeModeEnabled()) {
      const {
        execIntoTmuxWorktree
      } = await import('../utils/worktree.ts');
  ...
  } = await import('../utils/earlyInput.ts');
  startCapturingEarlyInput();
