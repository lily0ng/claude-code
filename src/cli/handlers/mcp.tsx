...
import { MCPServerDesktopImportDialog } from '../../components/MCPServerDesktopImportDialog.ts';
...
import { clearMcpClientConfig, clearServerTokensFromLocalStorage, getMcpClientConfig, readClientSecret, saveMcpClientSecret } from '../../services/mcp/auth.ts';
import { connectToServer, getMcpServerConnectionBatchSize } from '../../services/mcp/client.ts';
import { addMcpConfig, getAllMcpConfigs, getMcpConfigByName, getMcpConfigsByScope, removeMcpConfig } from '../../services/mcp/config.ts';
import type { ConfigScope, ScopedMcpServerConfig } from '../../services/mcp/types.ts';
import { describeMcpConfigFilePath, ensureConfigScope, getScopeLabel } from '../../services/mcp/utils.ts';
import { AppStateProvider } from '../../state/AppState.ts';
import { getCurrentProjectConfig, getGlobalConfig, saveCurrentProjectConfig } from '../../utils/config.ts';
import { isFsInaccessible } from '../../utils/errors.ts';
...
import { cliError, cliOk } from '../exit.ts';
...
export async function mcpServeHandler({
  ...
  try {
    ...
    const {
      startMCPServer
    } = await import('../../entrypoints/mcp.ts');
...
export async function mcpAddFromDesktopHandler(options: {
  ...
  try {
    ...
    } = await import('../../utils/claudeDesktop.ts');
